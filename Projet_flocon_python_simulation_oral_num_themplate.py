...
def frontbox(pos,dim_box):
    if ((pos[1]-(dim_box[1])/2)**2+(pos[0]-(dim_box[1])/2)**2<=((dim_box[1])/2-2)**2) \
        and ((pos[1]-(dim_box[1])/2)**2+(pos[0]-(dim_box[0])/2)**2>=(((dim_box[1])/2-6)**2)): 
...
def valapha(pos,mat_voi,dim_box,ice):
    ...
    if  V==1 and H==0:
        alpha=0.005 # valeur pour vertical (mi a 0 pour le moment pour les testes ''rapide'')
    elif V<=1 and H==1:
        alpha=0.2 # valeur pour horizatal et vertical petit  
    elif H==2 and V==0:
        alpha=0.2 #reduit pour aider la croisence de branche
    elif H==0 and V==0:
        alpha=0
    else:
        alpha=1
...
dim_box=np.array([120,120,20]).astype(np.int8) 
...
if True:
    ...
    while continuer1:
        ...
        # vertical
        if (fron_state==np.array([dim_box[0]-10,dim_box[1]-10,dim_box[2]-1,np.nan])).any() or (fron_state==np.array([5,5,1,np.nan])).any() or i3>1400:
            print('fin')
            saveframet(framet)    
            continuer1=False