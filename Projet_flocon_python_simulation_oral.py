...
def frontbox(pos,dim_box):
    if (pos[:2]==np.array([0,0])).any() or (pos[:2]==(dim_box[:2]-1)).any() :
        front_box = True
    else:
        front_box = False
    return front_box
...
dim_box=np.array([100,100,13]).astype(np.int32) 

center=(np.floor((dim_box)/2)).astype(np.int32)
center1=(center+np.array([0,0,1])).astype(np.int32)
center2=(center+np.array([0,0,-1])).astype(np.int32)
...
vap=0.1*sigma_limit*(np.ones(dim_box))
...
if True:
    ...
    while continuer1:
        ...
        # vertical
        if (fron_state==np.array([dim_box[0]-1,dim_box[1]-1,dim_box[2]-1,np.nan])).any() or (fron_state==np.array([1,1,1,np.nan])).any() or i3>650:
            print('fin')
            saveframet(framet)    
            continuer1=False
